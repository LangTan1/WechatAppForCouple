...
### v5.7 — 时光相册单张删除 + 滑动预览
- **单张照片删除**：相册内照片预览页新增“删除照片”入口
  - 删除前二次确认，避免误删
  - 只删除当前照片，保留相册和其它照片
  - 删除后若仍有照片，自动切到相邻照片；删空后关闭预览并保留空相册
- **连续看图体验优化**：
  - 照片预览支持上一张 / 下一张按钮
  - 支持左右滑动直接切换照片，无需关闭后重新选择
  - 切换照片时自动重置缩放状态和评论输入
- **图片同步链路保持不变**：
  - 继续保留 `cloud://` fileID 作为真实存储值
  - 展示层继续使用 `displayUrl` / `displayCoverUrl`
  - 不改动云函数和相册存储净化逻辑，避免临时 URL 回写污染云端
