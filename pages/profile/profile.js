...
Page({
  ...
  refreshData() {
    ...
    const date = new Date(togetherDate);
    const togetherDateStr = `${date.getFullYear()}年${date.getMonth() + 1}月${date.getDate()}日`;
    this.setData({
      isDev,
      ...
      coinBalance: storage.getGirlCoins(),
  ...
  changeMyAvatar() {
    wx.chooseImage({
      ...
      success: (res) => {
        ...
        const cloudPath = `avatars/my_${Date.now()}.${ext}`;
  ...
  changePartnerAvatar() {
    wx.chooseImage({
      ...
      success: (res) => {
        const tempPath = res.tempFilePaths[0];
        const ext = tempPath.split('.').pop() || 'jpg';
        const cloudPath = `avatars/partner_${Date.now()}.${ext}`;
        wx.showLoading({ title: '上传中...' });
        wx.cloud.uploadFile({
          cloudPath,
          filePath: tempPath,
  ...
  submitCoins() {
    ...
    const doChange = () => {
      ...
      wx.showToast({ title: isAdd ? `已发放 ${amount} 币` : `已扣除 ${amount} 币`, icon: 'none' });
    };
  ...
  onNewUserKey(e) {
    this.setData({ newUserKey: e.detail.value });
  },
  ...
  onTogetherDateChange(e) {
    this.setData({ togetherDateVal: e.detail.value });
  },
  ...
  goWeatherSettings() {
    wx.navigateTo({ url: '/pages/weather-settings/weather-settings' });
  },
  ...
  resetApp() {
    wx.showModal({
      title: '重新设置',
      content: '将删除云端情侣空间，对方也将无法继续使用。这个操作不可恢复。',
      confirmText: '确认重置',
      confirmColor: '#FF6B8A',
      success: (res) => {
  ...
  showInviteCode() {
    const code = storage.getLastInviteCode();
    if (!code) {
