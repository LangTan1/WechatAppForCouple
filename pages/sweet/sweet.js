...
Page({
  ...
  loadItems() {
    this.setData({
      items: storage.getSweet().map(d => ({ ...d })).reverse()
    });
  },
  ...
  addItem() {
    ...
    if (!newEvent.trim()) {
      wx.showToast({ title: '写一下什么让你心动～', icon: 'none' });
      return;
    }
    if (!newContent.trim()) {
    ...
    const items = storage.getSweet();
    items.push(newItem);
    storage.setSweet(items);

    this.setData({ showModal: false, newEvent: '', newContent: '' });
    this.loadItems();
    wx.showToast({ title: '心动存好了 💗', icon: 'none' });
  },
  ...
  deleteItem(e) {
    ...
    wx.showModal({
      title: '确认删除',
      content: '确定要删除这条心动记录吗？',
      confirmColor: '#FF6B8A',
      success: (res) => {
        if (res.confirm) {
          let items = storage.getSweet();
  ...
  async syncFromCloud() {
    try { await storage.loadFromCloud(); } catch (e) { console.error('[sweet] loadFromCloud failed:', e); }
    this.loadItems();
  }
});
