...
Page({
  ...
  loadItems() {
    this.setData({
      items: storage.getAngry().map(d => ({ ...d })).reverse()
    });
  },
  ...
  async addItem() {
    ...
    const items = storage.getAngry();
    items.push(newItem);
    storage.setAngry(items);

    this.setData({ showModal: false, newEvent: '', newContent: '' });
    this.loadItems();
    wx.showToast({ title: '发泄完了，翻篇！😤', icon: 'none' });
  },
